"""
main.py — The AI Stack
Daily AI/LLM/agentic-AI educational Shorts orchestrator.
"""
...
def main():
    print(f"\n{'='*60}")
    print(f"The AI Stack Bot  —  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*60}\n")

    os.makedirs(WORK_DIR, exist_ok=True)

    # 1. Pick today's topic
    print("[1/6] Picking today's topic (news + curated rotation)...")
    topic = pick_topic()

    # 2. Generate educational script + metadata
    ...
    # 4. Tech-aesthetic imagery
    print("\n[4/6] Fetching tech imagery...")
    images_dir  = os.path.join(WORK_DIR, 'images')
    pexels_key  = os.environ.get('PEXELS_API_KEY', '')
    image_paths = fetch_footage(
        topic_title=topic.get('topic_title') or topic.get('news', {}).get('title', 'AI'),
